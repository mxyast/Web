...
export function BrandForm() {
  ...
  return (
    <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-8 space-y-6">
      <div>
        <h2 className="text-sm font-black uppercase tracking-widest text-slate-800">Yeni Marka Ekle</h2>
        <p className="text-[11px] font-bold text-gray-400 mt-1">Ürünleriniz için yeni bir marka tanımlayın.</p>
      </div>

      <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 px-1">Marka Adı</label>
          <input
            required
            name="name"
            type="text"
            placeholder="Örn: Baseus"
        ...
        >
          ...
          {isPending ? "Kaydediliyor..." : "Marka Ekle"}
        </button>
      </form>
    </div>
  );
